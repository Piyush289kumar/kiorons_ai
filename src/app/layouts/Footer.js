...
export default function Footer() {
  const [appConfigMetaData, setAppConfigMetaData] = useState([])
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/app-config-meta-data`)
      .then((res) => res.json())
      .then((data) => {
        setAppConfigMetaData(data)
      })
    ...
    <footer className="@container mt-3xl mb-md">
      ...
      <div className="px-sm gap-md @md:gap-sm @md:flex-row @md:justify-between @md:items-center max-w-container flex w-full flex-col items-center">
        <div className="@md:w-auto @md:justify-start gap-sm @md:gap-2xs @lg:gap-sm flex justify-between">
          {appConfigMetaData.twitter_link && (
            <Link
              href={appConfigMetaData.twitter_link}
          ...
          {appConfigMetaData.youtube_link && (
            <Link
              href={appConfigMetaData.youtube_link}
              className="transition ease-curve-a duration-250"
              target="_blank"
              rel="noreferrer"
              aria-label="YouTube"
          ...
          {appConfigMetaData.linkedin_link && (
            <Link
              href={appConfigMetaData.linkedin_link}
              className="transition ease-curve-a duration-250"
              target="_blank"
              rel="noreferrer"
              aria-label="LinkedIn"
          ...
          {appConfigMetaData.instagram_link && (
            <Link
              href={appConfigMetaData.instagram_link}
              className="transition ease-curve-a duration-250"
              target="_blank"
              rel="noreferrer"
        ...
        <div>
          <span className="text-nav">Kiorons © 2025</span>
          <button
            type="button"
        ...
        <div className="relative">
          ...
          >
            <span className="text-cta gap-4xs flex-baseline flex">
              English<span className="text-primary-60">India</span>
            </span>
          </button>
        </div>
      </div>
